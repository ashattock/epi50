...
# Load results: main results by economy group
dt2 = try_load(o$pth$results, "detailed_results_economy") %>%
  arrange(disease) %>%
  mutate(disease = fct_inorder(disease), 
         economy = factor(economy, levels = c("Low economy", 
...
n_economy  = length(levels(dt2$economy))
...
pals = list(
  ...
  economy = colour_scheme(o$palette_economy, n = n_economy), 
...
## Total by disease for each economy group
```{r, economy_by_disease}

# NOTE: Figure 3 of paper

economy_d_dt <- dt2[, .(deaths_averted = sum(total)), by = .(economy, year, disease)]
full_dt <- data.table(expand.grid(
  economy = unique(economy_d_dt$economy),
  year = unique(economy_d_dt$year),
  disease = unique(economy_d_dt$disease)
))
economy_d_dt <- merge(economy_d_dt, full_dt, all.y = T)
economy_d_dt[is.na(deaths_averted), deaths_averted := 0]

g <- ggplot(economy_d_dt[year %in% o$future_years]) + 
  aes(x = year, y = deaths_averted / 1e6 , fill = disease) +
  geom_bar(position = "stack", stat = "identity") +
  facet_wrap(~economy)
...
# Save and print figure
fig_save(g, "economy_by_disease")
print(g)

```

## Total by year by economy
```{r, total_by_economy}

economy_y_dt <- dt2[, .(deaths_averted = sum(total)), by = .(economy, year)]

cast_dt <- dcast(economy_y_dt[year %in% o$future_years], economy ~ year)

g <- ggplot(economy_y_dt[year %in% o$future_years]) + 
  aes(x = year, y = deaths_averted / 1e6 , fill = economy) +
  geom_bar(position = "stack", stat = "identity")
...
# Save and print figure
fig_save(g, "total_by_economy")
print(g)

# Save and print table
write.csv(cast_dt, paste0(o$pth$results, "total_by_economy_table.csv"), row.names = F)
kable(cast_dt)

```

## Incremental by disease for each economy group
```{r, economy_by_disease_increment}

economy_d_dt <- dt2[, .(deaths_averted = sum(incremental)), by = .(economy, year, disease)]
full_dt <- data.table(expand.grid(
  economy = unique(economy_d_dt$economy),
  year = unique(economy_d_dt$year),
  disease = unique(economy_d_dt$disease)
...
# Save and print figure
fig_save(g, "economy_by_disease_increment")
print(g)

```

## Incremental by year by economy
```{r, increment_by_economy}

economy_y_dt <- dt2[, .(deaths_averted = sum(incremental)), by = .(economy, year)]
...
# Save and print figure
fig_save(g, "increment_by_economy")
print(g)
